-- Adiciona coluna de perfis permitidos nos módulos
-- Valores possíveis: 'consultor', 'gestor', 'outros'
-- Default: visível para consultor e gestor (comportamento atual)

ALTER TABLE modulos
  ADD COLUMN IF NOT EXISTS perfis_permitidos TEXT[] DEFAULT ARRAY['consultor', 'gestor'];

-- Garante que módulos já existentes herdam o default
UPDATE modulos
  SET perfis_permitidos = ARRAY['consultor', 'gestor']
  WHERE perfis_permitidos IS NULL;

-- Atualiza a RPC para filtrar por perfil consultor
...
BEGIN
  SELECT p.created_at, p.proxima_aula_liberada_em
  INTO v_ultima_aprovada_em, v_proxima_liberada_em
  FROM progresso p
  WHERE p.aluno_id = p_aluno_id AND p.aprovado = TRUE
  ...
  WHERE a.publicado = TRUE AND m.publicado = TRUE
    AND 'consultor' = ANY(m.perfis_permitidos)
    AND NOT EXISTS (
      SELECT 1 FROM progresso p2
      WHERE p2.aluno_id = p_aluno_id AND p2.aula_id = a.id AND p2.aprovado = TRUE
    )
  ...
  SELECT m.id, m.ordem, m.titulo, a.id, a.ordem, a.titulo, a.descricao,
    ...
    CASE
      WHEN EXISTS (
        SELECT 1 FROM progresso p WHERE p.aluno_id = p_aluno_id AND p.aula_id = a.id AND p.aprovado = TRUE
      ) THEN 'concluida'
