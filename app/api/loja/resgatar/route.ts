...
export async function POST(req: NextRequest) {
  ...
  const { data: premio } = await adminClient.from('premios')
  ...
  const { data: pontosRows } = await adminClient.from('aluno_pontos')
    .select('quantidade')
  ...
  const { data: resgate } = await adminClient.from('resgates')
  ...
  if (premio.quantidade_disponivel !== null) {
    await adminClient.from('premios')
