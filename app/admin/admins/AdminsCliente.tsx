...
type Admin = { id: string; nome: string; email: string; role: string; ativo: boolean | null; created_at: string | null; user_id: string }
...
export default function AdminsCliente({ adminsIniciais, meuUserId }: { adminsIniciais: Admin[]; meuUserId: string }) {
 ...
 <p style={{ color: 'var(--avp-text-dim)', fontSize: 13, marginBottom: 20 }}>Esta pessoa poderá logar em <strong style={{ color: 'var(--avp-text)' }}>{typeof window !== 'undefined' ? window.location.host : 'este painel'}</strong> e terá acesso completo ao painel.</p>
 ...
 Após criar, informe o <strong>e-mail</strong> e a <strong>senha</strong> para a pessoa. Ela acessa em <strong>{typeof window !== 'undefined' ? window.location.host : 'este painel'}</strong> e poderá alterar a senha depois.
 </div>
 ...
 <td style={{ padding: '14px 16px', color: 'var(--avp-text-dim)', fontSize: 13 }}>{a.created_at ? new Date(a.created_at).toLocaleDateString('pt-BR') : '—'}</td>
 <td style={{ padding: '14px 16px' }}>
 {a.user_id !== meuUserId && (
