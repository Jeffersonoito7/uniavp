...
export function GraficoRosca({
  ...
  return (
    <div style={{ background: 'var(--avp-card)', border: '1px solid var(--avp-border)', borderRadius: 12, padding: '20px 22px' }}>
      <p style={{ fontSize: 13, fontWeight: 700, color: 'var(--avp-text-dim)', marginBottom: 16, textTransform: 'uppercase', letterSpacing: '0.05em' }}>{titulo}</p>
      <div style={{ display: 'flex', alignItems: 'center', gap: 24, flexWrap: 'wrap' }}>
        <svg width={180} height={180} style={{ flexShrink: 0 }}>
          {arcos.map((a, i) => (
            <path key={i} d={a.d} fill={a.cor} opacity={0.9} />
          ))}
          <text x={cx} y={cy - 6} textAnchor="middle" fill="var(--avp-text)" fontSize={22} fontWeight={800}>{centro}</text>
          <text x={cx} y={cy + 14} textAnchor="middle" fill="var(--avp-text-dim)" fontSize={11}>total</text>
        </svg>
        <div style={{ flex: 1, minWidth: 140 }}>
          {arcos.map((a, i) => (
            <div key={i} style={{ display: 'flex', alignItems: 'center', gap: 10, marginBottom: 12 }}>
              ...
              <div style={{ flex: 1 }}>
                <p style={{ margin: 0, fontSize: 13, color: 'var(--avp-text)', fontWeight: 600 }}>{a.label}</p>
                <p style={{ margin: 0, fontSize: 12, color: 'var(--avp-text-dim)' }}>{a.valor} alunos ({a.pct}%)</p>
              </div>
            </div>
          ))}
        </div>
...
export function GraficoFunil({
  ...
  return (
    <div style={{ background: 'var(--avp-card)', border: '1px solid var(--avp-border)', borderRadius: 12, padding: '20px 22px' }}>
      <p style={{ fontSize: 13, fontWeight: 700, color: 'var(--avp-text-dim)', marginBottom: 20, textTransform: 'uppercase', letterSpacing: '0.05em' }}>{titulo}</p>
      {itens.map((it, i) => {
        ...
        return (
          <div key={i} style={{ marginBottom: 20 }}>
            <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: 6 }}>
              <div>
                <span style={{ fontSize: 13, fontWeight: 700, color: 'var(--avp-text)' }}>{it.label}</span>
                <span style={{ fontSize: 12, color: 'var(--avp-text-dim)', marginLeft: 8 }}>{it.sub}</span>
              </div>
              <span style={{ fontSize: 14, fontWeight: 800, color: it.cor }}>{it.valor}</span>
            </div>
            <div style={{ height: 8, background: 'var(--avp-border)', borderRadius: 4, overflow: 'hidden' }}>
              <div style={{ height: '100%', width: `${pct}%`, background: it.cor, borderRadius: 4, transition: 'width .4s' }} />
            </div>
            <p style={{ margin: '4px 0 0', fontSize: 11, color: 'var(--avp-text-dim)' }}>{Math.round(pct)}% dos ativos</p>
          </div>
        )
      })}
    </div>
...
export function GraficoMeses({ meses }: { meses: [string, number][] }) {
  ...
  return (
    <div style={{ background: 'var(--avp-card)', border: '1px solid var(--avp-border)', borderRadius: 12, padding: '20px 22px' }}>
      <p style={{ fontSize: 13, fontWeight: 700, color: 'var(--avp-text-dim)', marginBottom: 20, textTransform: 'uppercase', letterSpacing: '0.05em' }}>Conclusoes por mes</p>
        ...
        {visiveis.map(([mes, qtd]) => {
          ...
          return (
            <div key={mes} style={{ flex: 1, display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 4 }}>
              ...
              <span style={{ fontSize: 10, color: 'var(--avp-text-dim)', textTransform: 'capitalize' }}>{nomeMes(mes)}</span>
            </div>
          )
        })}
      </div>
      <div style={{ borderTop: '1px solid var(--avp-border)', paddingTop: 14 }}>
        {meses.map(([mes, qtd]) => {
          const [ano, m] = mes.split('-')
          const label = new Date(Number(ano), Number(m) - 1).toLocaleString('pt-BR', { month: 'long', year: 'numeric' })
          return (
            <div key={mes} style={{ display: 'flex', justifyContent: 'space-between', padding: '6px 0', borderBottom: '1px solid var(--avp-border)', fontSize: 13 }}>
              <span style={{ color: 'var(--avp-text-dim)', textTransform: 'capitalize' }}>{label}</span>
