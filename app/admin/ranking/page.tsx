...
export default async function RankingPage() {
 ...
 const { data: alunosTenant } = await tq(adminClient.from('alunos').select('id, nome'))
 const alunoIds = (alunosTenant ?? []).map((a: any) => a.id)

 const { data: pontos } = alunoIds.length > 0
   ? await adminClient.from('aluno_pontos').select('aluno_id, quantidade').in('aluno_id', alunoIds)
   : { data: [] }
 ...
 for (const a of alunosTenant ?? []) alunoMap[a.id] = a.nome
