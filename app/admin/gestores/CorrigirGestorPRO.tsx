    ...
    <div style={{ background: 'rgba(249,115,22,0.06)', border: '1px solid rgba(249,115,22,0.25)', borderRadius: 12, padding: '20px 22px', marginBottom: 28 }}>
      ...
      <div style={{ display: 'flex', gap: 8 }}>
        <input
          ...
          style={{ flex: 1, background: 'rgba(0,0,0,0.3)', border: '1px solid rgba(249,115,22,0.3)', borderRadius: 8, padding: '10px 12px', color: '#fff', fontSize: 14, outline: 'none' }}
        />
        <button
          onClick={verificar}
      ...
      {resultados && resultados.map(r => (
        <div key={r.gestor.id} style={{ marginTop: 16, background: 'rgba(0,0,0,0.25)', borderRadius: 10, padding: '16px 18px', border: r.corrigido ? '1px solid rgba(74,222,128,0.4)' : '1px solid rgba(255,255,255,0.08)' }}>
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', gap: 12, flexWrap: 'wrap', marginBottom: 12 }}>
            <div>
              <p style={{ fontWeight: 800, fontSize: 15, color: '#fff', margin: '0 0 2px' }}>{r.gestor.nome}</p>
              <p style={{ fontSize: 12, color: 'rgba(255,255,255,0.45)', margin: 0 }}>{r.gestor.email} · {r.gestor.whatsapp}</p>
            </div>
            {r.corrigido ? (
          ...
          <div style={{ display: 'flex', gap: 16, marginBottom: 10, flexWrap: 'wrap' }}>
            <div>
              <p style={{ fontSize: 11, color: 'rgba(255,255,255,0.35)', margin: '0 0 2px' }}>Status antes</p>
              <span style={{ fontSize: 13, fontWeight: 700, color: corCor(r.status_antes) }}>{r.status_antes}</span>
            </div>
            <div>
              <p style={{ fontSize: 11, color: 'rgba(255,255,255,0.35)', margin: '0 0 2px' }}>Vencimento antes</p>
            ...
            {r.corrigido && (
              <div>
                <p style={{ fontSize: 11, color: 'rgba(255,255,255,0.35)', margin: '0 0 2px' }}>Acao realizada</p>
                <span style={{ fontSize: 13, color: '#4ade80' }}>{r.acao}</span>
              </div>
            )}
          </div>

          <p style={{ fontSize: 12, fontWeight: 700, color: 'rgba(255,255,255,0.4)', margin: '8px 0 6px' }}>Pagamentos ({r.pagamentos.length})</p>
          <div style={{ display: 'flex', flexDirection: 'column', gap: 6 }}>
            {r.pagamentos.length === 0 && (
              <p style={{ fontSize: 12, color: 'rgba(255,255,255,0.4)' }}>Nenhum pagamento encontrado.</p>
            )}
            {r.pagamentos.map(p => (
              <div key={p.id} style={{ display: 'flex', gap: 12, alignItems: 'center', padding: '8px 10px', background: 'rgba(255,255,255,0.03)', borderRadius: 6, fontSize: 12 }}>
                <span style={{ fontWeight: 700, color: corPago(p.status), width: 70 }}>{p.status}</span>
                <span style={{ color: 'rgba(255,255,255,0.6)' }}>R$ {Number(p.valor ?? 0).toFixed(2)}</span>
                <span style={{ color: 'rgba(255,255,255,0.4)' }}>{p.plano_meses} mes{p.plano_meses > 1 ? 'es' : ''}</span>
