...
export default function LinkParceiroCardGestor({
  linkAtual,
  whatsapp,
  baseUrl,
}: {
  ...
  const linkCaptacao = `${baseUrl}/g/${whatsapp}`
  ...
  async function salvar(e: React.FormEvent) {
    ...
    const res = await fetch('/api/gestor/perfil', {
      ...
      body: JSON.stringify({ link_externo: link.trim() || null }),
    })
    if (res.ok) {
  ...
  return (
    <div style={{ background: 'var(--avp-card)', border: '1px solid var(--avp-border)', borderRadius: 14, padding: '18px 20px', marginBottom: 20 }}>
